...
4. 确认字体字号，标题必须大于36px，正文必须大于30px，禁止整体过小字号。
5. 禁止 emoji 与情绪符号。
...
* **并列信息块数量与网格（硬性）**：把「卡片 / 要点 / 图标+标题 / KPI 数字块」等算作并列单元。
  * **若同一 section 内并列单元数 ≥ 4**：**必须**用 **2×2 网格**（两行两列、四宫格），用 CSS Grid/Flex 实现四块大致均分可用区域；**禁止**排成 **单行四列（1×4）** 或 **单列四行（4×1）** 的长条。
  * 3 个：一行三列；2 个：一行两列或上下两格；1 个：单块居中放大或加入icon/图片左右排布。
* 代码框和文字不要溢出父容器（宁可换行）
* 适当引入网页icon和图形元素
从 brief 提炼：
必须：
