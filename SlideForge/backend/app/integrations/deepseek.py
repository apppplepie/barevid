...
# deepseek-chat 单次 completion 输出上限（与官方一致）；超过则按此裁剪后传入请求体。
_DEEPSEEK_CHAT_OUTPUT_CAP = 8192


def _clamp_chat_max_tokens(n: int) -> int:
    """将 max_tokens 限制在 DeepSeek chat 模型可接受范围内。"""
    if n <= 0:
        return _DEEPSEEK_CHAT_OUTPUT_CAP
    return max(1, min(int(n), _DEEPSEEK_CHAT_OUTPUT_CAP))
...
3. 每个 segments[].script 是一段完整口播，供语音合成使用，可含语气词但不要用舞台说明括号。**口播字数只约束 script**：单段建议约 50～900 字（可依素材微调）；若用户消息中另有「口播篇幅/字数」要求，以该要求为准，可围绕素材适当扩写或缩写以落在给定区间；扩写时不捏造事实与数据。
4. 每个 segments[].brief 为该段提炼/提要：优先保留代码、数据、观点与必要标签，字数少于300字，可与 script 长短独立（用途是页面/提纲/检索等，非口播朗读）。
...
7. 大标题 1～8 个；每标题下 1～5 个小段，视原文长度而定。"""
...
# 新建 project_styles 行时的占位 slug：库内非空，语义为「未选具体风格」，不是极光玻璃。
DEFAULT_DECK_STYLE_PRESET = "none"
...
DECK_STYLE_PRESETS: dict[str, dict[str, str]] = {
    ...
    "dark_neon": {
        ...
        "style_name": "温暖治愈",
        "detail_for_prompt": (
            "温暖治愈系前端风格，柔和渐变，低刺激配色，圆角卡片，轻插画与柔光效果，"
        ),
    },
    "material_design": {
        "label": "Material 质感（Material Design）",
        "style_name": "Material Design 质感",
        "detail_for_prompt": (
            "Material Design 风格前端，层级明确，组件统一，动效自然，卡片和浮层清晰，强调可用性与一致性"
        ),
    },
    "flat_illustration": {
        "label": "扁平插画风",
        "style_name": "扁平插画风",
        "detail_for_prompt": (
            "扁平化插画风前端界面，背景被黄蓝斜角对切割成色块，卡片、文字、图标撞色，极致艺术感"
        ),
    },
    "editorial_luxury": {
        ...
        "detail_for_prompt": (
            "杂志风、对比留白、精致分栏，高端名片，衬布深色，页面浅色，整体有质感"
        ),
    },
    "futuristic_hud": {
...
_DECK_CANVAS_BLOCK: dict[str, str] = {
    # "16:9": "16:9 横屏（1920×1080）。横向排版，大屏主机端 UI。禁止垂直布局超过3个并列单元，允许水平布局并列3个并列单元",
    # "4:3": "4:3 横屏（1024×768）。横向排版，桌面 UI。建议水平布局2-3个并列单元",
    # "9:16": (
    #     "9:16 竖屏（720×1280）。纵向排版，手机网页/App：单列纵向流；禁止水平并列布局，建议垂直布局2-3个并列元素"
    #     "标题≥42px，正文≥36px。卡片/组件宽≤屏宽 92%，左右边距≥4%。按钮高度≥80px。"
    # ),
    # "1:1": "1:1 方屏（1080×1080）。方形画幅内居中布局。禁止垂直布或水平局超过2个并列单元",
    "16:9": "16:9 横屏（1920×1080）。桌面/主机端 UI。禁止垂直布局3个并列单元",
    "4:3": "4:3 横屏（1024×768）。桌面 UI。禁止并列布局3个并列单元",
    "9:16": (
        "9:16 竖屏（720×1280）。手机 App / 手机UI；老年模式、大字、关键词换行：单列纵向流。"
        "标题≥44px，正文≥40px。卡片/组件宽≤屏宽 92%，左右边距≥4%。按钮高度≥80px。"
    ),
    "1:1": "1:1 方屏（1080×1080）。方形画幅内居中布局。禁止并列布局3个并列单元",
}
...
STYLE_PROMPT_MASTER_SYSTEM = """根据下列信息写一段纯文本的统一视觉风格提示词，给AI做前端设计用。
...
用户补充（选填，未提供时可为空）：
{user_hint}
当风格类型、风格说明或用户补充为空、为「（无）」或未提供时，不要强行套用不存在的约束；不要强行描述项目标题，确定统一、可执行的视觉风格。
要求（必须遵守）：
1. 只输出一段连续正文，不要用 Markdown 标题/列表/代码块；不要输出 JSON。禁止 emoji 与情绪符号。
2. 明确主色/辅色/强调色、前景颜色，背景颜色，背景（纯色/渐变/光效）、字体层级、组件质感（圆角/阴影等）、动效（进入/微动）、装饰元素；若项目标题有明确主题，视觉气质需与之协调（仍遵守下方字号与禁止项）。
4. 确认字体字号，标题必须大于32px，正文必须大于28px，禁止整体过小字号。
5. 禁止 emoji 与情绪符号。
6. 篇幅控制在 300～900 字，避免空话。
"""
...
def _extract_single_page_draft_fallback(text: str) -> dict[str, Any] | None:
    """json.loads 失败时，用引号扫描抽取 main_title / html（应对裸换行、围栏等导致的非严格 JSON）。"""
    t = (text or "").strip()
    if not t:
        return None
    k = t.find('"main_title"')
    ...
    if qh < 0:
        return None
    try:
        html, _end_h = _scan_json_string(t, qh)
    except RuntimeError:
        return None
    ...
    try:
        ...
        if start != -1 and end != -1 and end > start:
            ...
            try:
                ...
                fb = _extract_single_page_draft_fallback(text)
                if fb is not None:
                    return fb
                raise RuntimeError(
...
async def _extract_json_object_with_repair(
    *,
    ...
    try:
        ...
        try:
            repair_mt = _clamp_chat_max_tokens(
                int(settings.deepseek_deck_max_tokens or _DEEPSEEK_CHAT_OUTPUT_CAP),
            )
            repair_mt = max(4096, repair_mt)
    ...
    try:
        completion = await _await_with_timeout(
            client.chat.completions.create(
                ...
                temperature=temp,
                max_tokens=_clamp_chat_max_tokens(
                    int(settings.deepseek_structure_max_tokens),
                ),
            ),
            timeout_seconds=settings.deepseek_request_timeout_seconds,
...
非必要不使用图片。仅在“信息需要视觉承载”时使用图片（如：著名公司商标，著名人物，著名地标，著名事件），纯数据/逻辑内容禁止使用图片。
图片必须与内容强相关，禁止出现原文“河水”配“潜水”的图片。
图片要大小合适，禁止小图。
优先使用稳定来源（如 unsplash、picsum、pexels）。禁止使用占位说明或无效链接。
一页允许1-2张图，但是每张图必须在同一位置叠加 3 张图片作为冗余（多层叠图）层级用 z-index 区分，顶部为最相关图片，底部为英文备用图，防止图片失效。
...
DECK_CORE_VISUAL_SYSTEM_BETA = (
    """你是资深前端工程师，负责生成全屏滚动着陆页的完整 HTML/CSS。禁止 emoji，禁止表情符号。

目标
- 输出现代、克制、有层次的全屏分屏页面
- 使用整屏 snap 滚动，支持鼠标滚轮切换
- 右侧必须有圆点导航
- 每个 section 高度为 100vh，整体无滚动条

内容规则
- 输出恰好 N 个 section
- 每个 section 使用 data-key="section-0" ... 连续编号
- 每个元素内文字不超过 20 个字
- 所有展示文字必须显式设置 font-size，且严格大于 28px
- 禁止大段 <p>、口语化句子、冗长解释
- 必须包含短标题、关键词高亮 <span>、模块化结构
- 不能只靠文字，必须有视觉元素参与表达

视觉规则
- 页面必须有设计感，对称平衡，禁止空洞排版
- 优先使用渐变、光晕、模糊、噪点、发光边缘、动效
- 动效可用 fade、slide、glow、float、scale
- 信息尽量用数字、对比条、进度、趋势图式表达
- 元素宽度不得小于 40px
- 版面必须平衡，内容不能溢出页面

技术偏好
- 优先使用 React Icons：hi、lu、tb
- 优先用 CSS scroll-snap + 锚点实现切换
- 尽量少用内联 JS
...
# 实验：更偏艺术策展 / 电影感叙事的视觉指令（结构约束与正式版一致，便于 A/B）
DECK_CORE_VISUAL_SYSTEM = (
    """你是视觉优先的前端工程师：你在交付**全屏滚动着陆页**（full viewport、整屏 snap 滚动），用完整 HTML/CSS 实现，禁止 emoji，禁止表情符号。
...
* 禁止 emoji / 情绪符号
* 页面文本必须是“展示文案”，非口语/解说
* 字号下限（不可协商）：凡面向用户阅读的展示文字（标题、副标题、列表项、按钮/标签、高亮关键词、引用等），在 CSS 中须显式 font-size，且数值须 **严格大于 30px**；禁止依赖默认 body 字号、禁止用 rem/em 间接导致折算后 ≤30px。代码块内等宽字体可略小，但代码块外的说明文字仍须 >30px。自检：在样式中搜索 font-size，任一展示文本若 ≤30px 即视为不合格输出。
* 每个元素内文字不超过 30 个字
表现力规则（必须）
用“视觉”替代情绪表达：
* 图标（SVG）
...
* **严禁**在 JSON 的 html 字符串字面量内直接按回车换行：换行必须写成 \\n；**严禁**使用 Markdown 代码围栏包裹整段输出；否则后端无法解析。"""
...
def build_deck_system_for_generation(
    *,
    ...
    core = DECK_CORE_VISUAL_SYSTEM_BETA if use_beta_visual else DECK_CORE_VISUAL_SYSTEM
    ...
    try:
        completion = await _await_with_timeout(
            client.chat.completions.create(
                ...
                temperature=0.55,
                max_tokens=_clamp_chat_max_tokens(
                    int(settings.deepseek_style_max_tokens),
                ),
            ),
...
async def generate_deck_pages_html(
    *,
    ...
    system = build_deck_system_for_generation(
        ...
        use_beta_visual=use_beta_visual,
    )
    client = AsyncOpenAI(
        api_key=settings.deepseek_api_key,
        base_url=settings.deepseek_base_url,
    ...
    if deck_max > 0:
        create_kwargs["max_tokens"] = _clamp_chat_max_tokens(deck_max)
    try:
        completion = await _await_with_timeout(
...
async def generate_contextual_page_draft(
    *,
    ...
    try:
        deck_mt = int(settings.deepseek_deck_max_tokens or _DEEPSEEK_CHAT_OUTPUT_CAP)
        deck_mt = max(4096, _clamp_chat_max_tokens(deck_mt))
        completion = await _await_with_timeout(
            client.chat.completions.create(
                model=settings.deepseek_model,
                messages=[
