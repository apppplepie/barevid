...
def _append_target_narration_constraint(raw_text: str, target_seconds: int | None) -> str:
    """在用户素材后附加口播总时长约束（秒 → 约汉字数，供模型控制 script 总量）。"""
    text = (raw_text or "").strip()
    if not text:
        return text
    if target_seconds is None or target_seconds <= 0:
        return text
    # 中文口播经验值：约 3.5～4.5 字/秒，取 4 便于心算
    approx_chars = max(1, round(target_seconds * 4))
    return (
        f"{text}\n\n"
        f"【口播时长约束】用户希望整段口播（所有小段的 script 连续朗读）合成后总时长约 {target_seconds} 秒。"
        f"上述素材整理后，全部 segments[].script 拼接的总字数建议控制在约 {approx_chars} 个汉字左右"
        "（按约每秒 4 字估算，可略浮动）。"
        "若素材不足则适当增加展开、过渡与总结；若过长则精简、合并段落。"
        "必要时减少大标题数量或小标题数量，以优先满足总时长感。"
    )
...
def resolve_deck_style_preset(preset_id: str) -> dict[str, str]:
    ...
    if not meta:
        raise RuntimeError(
            f"未知风格预设 {preset_id!r}，可选：{', '.join(DECK_STYLE_PRESET_ORDER)}"
        )
    return meta


# 生成单页 deck 时注入 system（与 services.deck._PAGE_SIZE_META 一致）
_DECK_CANVAS_BLOCK: dict[str, str] = {
    "16:9": "16:9横屏（1920×1080）。桌面/主机端UI",
    
    "4:3": "4:3横屏（1024×768）。桌面UI",
    
     "9:16": "9:16竖屏（1080×1920）。手机端网页/App：单列纵向流。单行≤8字，超出必须换行；每段≤3行。标题≥42px，正文≥36px。卡片/组件宽度≤屏幕92%，左右留边≥4%。按钮高度≥80px。",
    
    "1:1": "1:1方屏（1080×1080）。方形画框内居中布局",
}
...
STYLE_PROMPT_MASTER_SYSTEM = """你是UIUE美术设计师。根据下列「风格类型 / 说明 / 用户补充」写一段纯文本的「统一视觉风格说明」。
...
2. 必须具体可执行：写清主色/辅色/强调色、背景（纯色/渐变/光效）、字体层级、组件质感（圆角/阴影等）、动效（进入/微动）、装饰元素。
3. 字号必须大于32px！！！！
4. 禁止 emoji 与情绪符号
...
async def structure_raw_text(
    raw_text: str,
    *,
    target_narration_seconds: int | None = None,
) -> StructuredPodcast:
    if not settings.deepseek_api_key:
        raise RuntimeError("未配置 DEEPSEEK_API_KEY")

    user_payload = _append_target_narration_constraint(raw_text, target_narration_seconds)
...
async def generate_style_prompt_text(
    *,
    ...
    system = (
        STYLE_PROMPT_MASTER_SYSTEM.replace("{style_name}", meta["style_name"])
        .replace("{style_detail}", meta["detail_for_prompt"])
