...
4. 每个 segments[].brief 为该段提炼/提要：尽量完整保留代码、数据、观点与必要细节，**不设字数上限**，可与 script 长短独立（用途是页面/提纲/检索等，非口播朗读）。
...
DECK_STYLE_PRESETS: dict[str, dict[str, str]] = {
    ...
    "flat_illustration": {
        ...
        "style_name": "扁平插画风",
        "detail_for_prompt": (
            "扁平化插画风前端界面，背景被两种对比色切割成色块，卡片、文字、图标撞色，无透明度无渐变，极致艺术感"
        ),
    },
    "editorial_luxury": {
        ...
        "detail_for_prompt": (
            "杂志风、对比留白、精致分栏，高端名片，衬布深色，页面前景浅色，整体有质感"
        ),
    },
    "futuristic_hud": {
...
_DECK_CANVAS_BLOCK: dict[str, str] = {
    ...
    "9:16": (
        "9:16 竖屏（1080×1920）。纵向排版，手机网页/App：单列纵向流；单行≤10 字，超出换行；禁止水平并列布局，建议垂直布局2-3个并列元素，所有字体必须大于36px！！！"
    ),
...
2. 写清主色/辅色/强调色、前景颜色，背景颜色，背景（纯色/渐变/光效）、字体层级、组件质感（圆角/阴影等）、动效（进入/微动）、装饰元素；若项目标题有明确主题，视觉气质需与之协调（仍遵守下方字号与禁止项）。
...
DECK_CORE_VISUAL_SYSTEM_BETA = (
        """你是资深前端工程师与视觉导演，生成全屏滚动着陆页 HTML/CSS。禁止 emoji。字号必须大于28px！！！

目标
- 展览级视觉（非网页、非PPT）
- 强构图、层次、空间感
- snap 滚动 + 右侧圆点导航 + 每屏 100vh

内容
- 每段文字 ≤20字，font-size >28px
- 禁止大段文本
- 必须：短标题 + <span>高亮 + 模块化
- 文字不能单独存在，必须绑定视觉
- 文字单元必须处在z-index=100的层级

视觉（核心）
- 每个 section = 一张完整画面（禁止PPT结构）
- 必须包含：
  - 1 主视觉锚点（几何体/图形/装置/图表）
  - 前景 / 中景 / 背景 三层
  - ≥3类元素（图标/数字/线条/图表/卡片/光晕/网格）
- 文字必须撑开父容器
- 必须有主次对比（大小/位置/密度）
...
动效
- 仅允许：fade / slide / float / 轻微视差 / 呼吸光
- 夸张动画

风格细节
- 图标必须有容器（发光底/卡片/边框），禁止裸放
- 背景必须分层（底色+光晕+噪点/纹理）
- 信息优先转为：数字/进度/对比/图形
...
输出标准
- 页面必须看起来像一套经过艺术指导的高完成度视觉作品
- 每个 section 都要有自己的“画面记忆点”
- 整体视觉必须统一，但每个 section 的构图和视觉焦点要有变化
- 结果要像展览海报、品牌大片、编辑设计，而不是演示文稿
