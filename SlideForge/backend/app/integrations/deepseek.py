...
_DECK_CANVAS_BLOCK: dict[str, str] = {
    ...
    "9:16": (
        ...
        "标题≥36px，正文≥28px。卡片/组件宽≤屏宽 92%，左右边距≥4%。按钮高度≥80px。"
    ),
...
4. 确认字体字号，标题必须大于36px，正文必须大于28px，禁止整体过小字号。
5. 禁止 emoji 与情绪符号。
6. 篇幅控制在 300～800 字，避免空话。
"""
...
仅在“信息需要视觉承载”时使用图片（如：产品商标、真实环境、人像、截图）；纯概念/数据/逻辑内容禁止使用图片。
图片要大小合适，不要太小。
...
* 禁止 emoji / 情绪符号
* 页面文本必须是“展示文案”，非口语/解说
* 字号下限（不可协商）：凡面向用户阅读的展示文字（标题、副标题、列表项、按钮/标签、高亮关键词、引用等），在 CSS 中须显式 font-size，且数值须 **严格大于 28px**（例如至少 38px）；禁止依赖默认 body 字号、禁止用 rem/em 间接导致折算后 ≤28px。代码块内等宽字体可略小，但代码块外的说明文字仍须 >28px。自检：在样式中搜索 font-size，任一展示文本若 ≤28px 即视为不合格输出。
表现力规则（必须）
用“视觉”替代情绪表达：
* 图标（SVG）
* 几何装饰（渐变 / 光晕 / 模糊）
...
* 排版（大标题 / 关键词高亮）
每个 section：
* 视觉元素丰富，不要纯文字块，不要过度装饰。
* 尽量填满屏幕、减少大块空白；主内容区在视口内水平垂直居中，整体重心平衡。
* **并列信息块数量与网格（硬性）**：把「卡片 / 要点 / 图标+标题 / KPI 数字块」等算作并列单元。
* **同一 section 内并列单元数必须 ≤ 3**
* 代码框和文字不要溢出父容器（宁可换行）
* 适当引入网页icon和图形元素
从 brief 提炼：
必须：
