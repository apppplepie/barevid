...
DECK_STYLE_PRESETS: dict[str, dict[str, str]] = {
    ...
    "material_design": {
        ...
        "detail_for_prompt": (
            "Material Design 风格前端，层级明确，组件统一，动效自然，卡片和浮层清晰，"
            "强调可用性与一致性"
        ),
    },
    "flat_illustration": {
...
_DECK_CANVAS_BLOCK: dict[str, str] = {
    "16:9": "16:9 横屏（1920×1080）。横向排版，大屏主机端 UI。",
    "4:3": "4:3 横屏（1024×768）。横向排版，桌面 UI。",
    "9:16": (
        "9:16 竖屏（1080×1920）。纵向排版，手机网页/App：单列纵向流；单行≤8 字，超出换行；每段≤3 行。"
        "标题≥32px，正文≥28px。卡片/组件宽≤屏宽 92%，左右边距≥4%。按钮高度≥80px。"
    ),
...
仅在“信息需要视觉承载”时使用图片（如：产品商标、真实环境、人像、截图），纯数据/逻辑内容禁止使用图片。非必要不使用图片。
图片要大小合适，禁止小图。
