...
DECK_STYLE_PRESETS: dict[str, dict[str, str]] = {
    ...
    "dark_neon": {
        ...
        "style_name": "温暖治愈",
        "detail_for_prompt": (
            "温暖治愈系前端风格，柔和渐变，低刺激配色，圆角大卡片，轻插画与柔光效果，"
            "界面亲和友善，适合情绪陪伴、生活记录类产品。"
        ),
    },
    "material_design": {
    ...
    "flat_illustration": {
        ...
        "style_name": "扁平插画风",
        "detail_for_prompt": (
            "扁平化插画风前端界面，色彩明快，造型简洁，视觉轻松，适合新手友好型产品，"
            "支持图标、插画、引导页和空状态设计。"
        ),
    },
    "editorial_luxury": {
...
_DECK_CANVAS_BLOCK: dict[str, str] = {
    "16:9": "16:9 横屏（1920×1080）。横向排版，桌面/主机端 UI。",
...
在“信息需要视觉承载”时使用图片（如：产品商标、真实环境、人像、截图），纯数据/逻辑内容禁止使用图片。
图片要大小合适，不要太小。
...
DECK_CORE_VISUAL_SYSTEM = (
    """你是负责设计高端产品网页UIUE工程师：你在交付**全屏滚动着陆页**（full viewport、整屏 snap 滚动），用完整 HTML/CSS 实现。
...
数据化表达：数字、对比条、进度、趋势、KPI
排版层级：大标题、关键词高亮、局部放大
结构化组件：卡片、标签、时间轴、步骤条、仪表盘、图文组合块
2）每个 section 必须“高端产品网页”风格
每个 section 都要包含至少一种非文本视觉元素，并形成内容 + 视觉的组合，而不是整块文字说明。
3）空间与构图
内容应在视口中形成平衡构图，避免单边堆满文字。
主内容区可以居中，但必须有层次变化。
页面应尽量填满屏幕，但不能靠堆文字实现“填充感”。
4）并列单元控制
将以下内容视为“并列单元”：
卡片，要点块，图标 + 标题，KPI 数字块，标签组，数据对比块
硬性要求：
同一 section 内并列单元数 ≤ 3
...
* 配色 / 背景 /布局 / 字体气质
* 组件质感 / 动画语言 / 装饰风格
* 展示文字字号：全局与逐元素样式均须满足 >28px（与上文一致，不可省略）
禁止整体风格偏离
