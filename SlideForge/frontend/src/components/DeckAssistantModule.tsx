...
export function DeckAssistantModule({
  ...
  if (!clip || clip.type !== 'video') {
    return (
      <div className="sf-ai-assistant-panel flex min-h-0 flex-1 flex-col items-center justify-center gap-2 p-6 text-center text-sm text-sf-muted light:text-slate-600">
        <Sparkles className="h-8 w-8 shrink-0 text-blue-500/40" />
        <p>
          双击时间轴<strong className="text-zinc-300 light:text-slate-800">视频轨</strong>
          上的演示大页，即可在此编辑该页的 HTML 草稿。
        </p>
      </div>
    );
  }
    ...
    <div className="sf-ai-assistant-panel flex min-h-0 min-w-0 flex-1 flex-col">
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-4">
        <div className="grid min-h-0 flex-1 grid-cols-2 gap-3">
          <div className="flex min-h-0 flex-col rounded-lg border border-blue-500/30 bg-blue-950/10 p-3 light:border-sky-300/50 light:bg-sky-50/90">
          ...
          <div className="flex min-h-0 flex-col rounded-lg border border-emerald-500/30 bg-emerald-950/10 p-3 light:border-emerald-300/45 light:bg-emerald-50/80">
            <div className="mb-2 flex shrink-0 items-center justify-between gap-2">
              ...
              <button
                ...
                className="inline-flex h-6 items-center justify-center rounded border border-emerald-400/30 px-2 text-[11px] text-emerald-200 transition-colors hover:bg-emerald-500/15 disabled:opacity-50 light:text-emerald-800 light:hover:bg-emerald-100"
              >
        ...
        {errorText ? (
          <div className="shrink-0 rounded border border-rose-500/30 bg-rose-950/20 px-2 py-1.5 text-xs text-rose-200 light:bg-rose-50 light:text-rose-800">
            {errorText}
          </div>
        ) : null}
        {isGenerating ? (
          <div className="shrink-0 text-xs sf-text-muted">AI 正在生成草稿…</div>
        ) : null}
      </div>

      <div className="border-t sf-border-base sf-bg-card p-4">
        <div className="flex flex-wrap items-end gap-x-2 gap-y-2">
          ...
          >
            <textarea
              ...
              rows={1}
              className="sf-input-control box-border min-h-11 max-h-[220px] w-full resize-none overflow-y-auto rounded-lg border px-3 py-2.5 text-sm leading-snug transition-all focus:border-blue-500/50 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
            />
          </form>
          <div className="flex min-h-11 shrink-0 flex-wrap items-end justify-end gap-2 sm:flex-nowrap">
            ...
            <button
              ...
              disabled={!hasDraft || isGenerating}
              className="box-border flex h-11 min-w-[100px] flex-1 items-center justify-center gap-2 rounded-lg border border-[var(--sf-border-base)] px-3 text-sm sf-text-primary transition-colors hover:bg-[var(--sf-chip-neutral-hover-bg)] disabled:opacity-50 sm:flex-initial sm:min-w-[110px]"
            >
