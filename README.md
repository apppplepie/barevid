...
省钱的代价是 **时间**。从「各段合成好了」到「手里有一个可下载的成片」，中间还要 **导出编码（常见是 Worker 按真实时间轴录屏 + ffmpeg）** 和 **传到你本地**，整体体感经常是：**等多久 ≈ 成片时长的 1.5 倍左右**（机器负载、分辨率、网络会有浮动）。也就是10min的时间可能需要15min才能合成好，视频越长合成的时间越长，如果要自行调整将会耗时更长，如果你懒得登上网站等待，可以找我们网站最下方的**咸鱼代做**（当然这是收费的）。
