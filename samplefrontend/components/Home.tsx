...
> & {
  ...
  includeIntro?: boolean;
  includeOutro?: boolean;
  /** 演示风格用户补充提示词，对应后端 deck_style_user_hint */
  deckStyleUserHint?: string;
};
...
function pipelineStatusVariant(project: Project): TagVariant {
  const st = (project.serverStatus || '').toLowerCase();
  const pl = project.pipeline;
  if (st === 'failed') {
    return 'danger';
...
function DeckMasterBadge({ project }: { project: Project }) {
  ...
  return (
    <Tag
      variant={reused ? 'violet' : 'neutral'}
      className="sf-deck-master-badge shrink-0 font-mono text-[11px] tabular-nums leading-none"
...
export function Home({
    ...
    try {
      await onCreateProject({
        ...
        narrationTargetSeconds:
          resolvedNarrationSeconds != null
            ? clampNarrationSeconds(resolvedNarrationSeconds)
            : undefined,
        deckStyleUserHint:
          deckMasterMode === 'reuse'
            ? undefined
  ...
  return (
    <div className="sf-home-root flex-1 bg-zinc-950 sf-bg-base text-zinc-100 sf-text-primary overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto space-y-12">
        
        {/* Header & Create Form */}
        <section className="space-y-6">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-100 sf-text-primary">工程管理</h1>
            <p className="text-zinc-400 sf-text-secondary mt-2">创建和管理你的视频工程项目。</p>
          </div>
          {currentUserId == null ? (
            <div className="rounded-2xl border border-zinc-800/80 sf-border-base bg-zinc-900/40 sf-bg-card px-6 py-14 text-center">
              <p className="text-sm text-zinc-300">
                请先登录后再使用创建项目、编辑工程与云共享等功能。
              </p>
              <p className="mt-3 text-xs text-zinc-500 sf-text-muted">点击右上角「Login」登录或注册账号。</p>
            </div>
          ) : (
            <>
              {manageError ? (
            ...
            className="sf-home-create-form relative overflow-hidden rounded-2xl border border-zinc-800/80 sf-border-base bg-gradient-to-b from-zinc-900/80 to-zinc-950/90 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)_inset] flex flex-col gap-6"
          >
            <div
            ...
            <div className="relative space-y-2">
              <label className="flex flex-wrap items-center gap-2 text-sm font-medium text-zinc-300">
                ...
                <span className="inline-flex items-center gap-2">
                  项目名称
                  <Tag variant="danger" className="px-1.5 text-[10px] font-semibold tracking-wide">
                    必填
                  </Tag>
                </span>
              </label>
            ...
            <div className="relative space-y-6">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-2 text-sm font-medium text-zinc-300">
                  <Video className="h-3.5 w-3.5 text-cyan-400/90" />
                  导出附加片头/片尾
                </label>
                <div className="flex flex-wrap gap-3">
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-zinc-800/90 bg-zinc-950/50 px-3 py-2 text-sm text-zinc-300 has-[:disabled]:cursor-not-allowed">
                    <input
                      type="checkbox"
                    ...
                    <span
                      aria-hidden
                      className="flex h-4 w-4 shrink-0 items-center justify-center rounded border border-zinc-600 bg-zinc-800/95 text-zinc-100 shadow-inner peer-checked:border-purple-500/70 peer-checked:bg-purple-600 peer-focus-visible:outline-none peer-focus-visible:ring-2 peer-focus-visible:ring-purple-500/45 peer-disabled:opacity-50 [&>svg]:scale-90 [&>svg]:opacity-0 peer-checked:[&>svg]:opacity-100"
                    >
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-1.5">
                    <Tag variant={pipelineStatusVariant(project)}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 mt-2 text-xs text-zinc-500">
                    ...
                    project.ownerUserId === currentUserId ? (
                      <Tag
                        variant="warning"
                        className="sf-chip-locked px-1.5 text-[10px]"
                      >
                        我上传
                      </Tag>
                    ) : null}
      ...
      {renameDialog ? (
        <div className="sf-modal-backdrop-medium fixed inset-0 z-40 flex items-center justify-center p-4">
