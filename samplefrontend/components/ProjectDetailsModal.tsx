...
function Section({
  ...
  return (
    <section className="space-y-2">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-zinc-500 sf-text-muted">{title}</h3>
      <div className="divide-y divide-zinc-800/80 rounded-xl border border-zinc-800/80 sf-border-base bg-zinc-950/50 sf-bg-card">
        {children}
      </div>
    </section>
  );
...
function Row({
  ...
  return (
    <div className="flex flex-col gap-0.5 px-3 py-2.5 sm:flex-row sm:items-start sm:gap-4">
      <div className="shrink-0 text-[13px] text-zinc-500 sf-text-muted sm:w-44">{label}</div>
      <div className="relative min-w-0 flex-1">
        ...
        <div className={`break-words text-[13px] text-zinc-200 ${showCopy ? 'pr-9' : ''}`}>{value}</div>
      </div>
    </div>
  );
}
...
function renderDescriptionColumn(raw: string | null): ReactNode {
  if (!raw?.trim()) return <span className="text-zinc-500">—</span>;
  const t = raw.trim();
      ...
      return (
        <div className="space-y-2">
          <p className="text-[11px] leading-relaxed text-zinc-500">
            数据库 <code className="text-zinc-400">description</code> 列中的{' '}
            <code className="text-zinc-400">__sfmeta:</code> JSON，与下方「母版与导出选项」一致；与口播主题（
            <code className="text-zinc-400">input_prompt</code>）无关。
          </p>
          <pre className="sf-code-block max-h-48 overflow-auto whitespace-pre-wrap rounded-lg border p-3 font-mono text-[12px] leading-relaxed text-zinc-300">
...
function LongText({ text }: { text: string }) {
  if (!text.trim()) return <span className="text-zinc-500">—</span>;
  return (
    <pre className="sf-code-block max-h-48 overflow-auto whitespace-pre-wrap rounded-lg border p-3 text-[12px] leading-relaxed text-zinc-300">
      {text}
    </pre>
  );
}
...
function NarrationScriptColumn({ outline }: { outline: unknown[] }) {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col lg:max-h-[min(64vh,560px)]">
      <div className="mb-3 flex shrink-0 flex-col gap-2 border-b border-zinc-800/80 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <h4 className="text-lg font-semibold text-zinc-100">口播台词</h4>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex cursor-pointer items-center gap-2 text-xs text-zinc-400">
            <input
              ...
              className="h-3.5 w-3.5 appearance-none rounded border border-zinc-600 bg-zinc-800 checked:border-purple-500 checked:bg-purple-500 text-purple-500 focus:ring-purple-500/40"
            />
            显示概括
          </label>
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
        {pages.length === 0 ? (
          <p className="text-sm text-zinc-500">暂无 page/step 口播结构（可能尚未结构化或大纲为空）。</p>
        ) : (
          pages.map((pg, pi) => (
            <div
              key={pg.page_node_id}
              className="space-y-3 rounded-xl border border-zinc-800/90 bg-zinc-900/40 p-3"
            >
              <div className="text-xs font-medium text-zinc-500">大标题 {pi + 1}</div>
              <div className="rounded-lg border border-zinc-800/80 bg-zinc-950/60 px-3 py-2 text-sm font-medium text-zinc-100">
              ...
              {pg.segments.map((s) => (
                ...
                >
                  <div className="text-xs text-zinc-500">小标题</div>
                  <div className="rounded-lg border border-zinc-800/80 bg-zinc-950/50 px-3 py-2 text-sm text-zinc-200">
                    {s.subtitle.trim() || '—'}
                  </div>
                  <div className="text-xs text-zinc-500">口播正文</div>
                  <div className="whitespace-pre-wrap rounded-lg border border-zinc-800/80 bg-zinc-950/50 px-3 py-2 text-sm leading-relaxed text-zinc-200">
                    {s.narration_text.trim() || '—'}
                  </div>
                  {showBrief ? (
                    <>
                      <div className="text-xs text-zinc-500">概括（可选）</div>
                      <div className="whitespace-pre-wrap rounded-lg border border-zinc-800/60 bg-zinc-950/30 px-3 py-2 text-sm leading-relaxed text-zinc-400">
...
export function ProjectDetailsModal({ open, onClose, projectId }: ProjectDetailsModalProps) {
  ...
  function renderTabContent() {
    ...
    switch (activeTab) {
          ...
          <div className="space-y-6">
            <Section title="主题与生成配置">
              <Row
                label="口播目标体量（秒）"
                value={
                  p.narration_target_seconds != null
                    ? String(p.narration_target_seconds)
                    : '—'
                }
              />
      ...
      case 'prompt_script':
        return (
          <div className="min-h-0 min-w-0 rounded-xl border border-zinc-800/80 bg-zinc-950/40 p-4">
                    ...
                    <Row
                      ...
                      value={
                        v === null || v === undefined ? (
                          ...
                          <pre className="sf-code-block max-h-36 overflow-auto whitespace-pre-wrap rounded-lg border p-2 text-[11px] text-zinc-400">
    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-zinc-800 bg-zinc-950/50 px-6 py-4">
              <h2 id="project-details-title" className="truncate text-lg font-semibold text-zinc-100">
                项目详情{p ? `：${p.name}` : ''}
              </h2>
              <button
                type="button"
            ...
            <div className="flex min-h-0 flex-1 overflow-hidden">
              <div className="flex w-64 shrink-0 flex-col gap-2 overflow-y-auto border-r border-zinc-800 bg-zinc-950/30 p-4">
                {TABS.map((tab) => {
                  const Icon = tab.icon;
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 rounded-lg border px-4 py-3 text-left text-sm font-medium transition-colors ${
                        disabled
                          ? 'cursor-not-allowed border-transparent text-zinc-600 opacity-50'
                          : isActive
              ...
              <div className="min-w-0 flex-1 overflow-y-auto bg-zinc-900 p-8">
                {loading && (
