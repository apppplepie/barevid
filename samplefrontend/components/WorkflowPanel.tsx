...
function PipelineNodeCard({
  ...
  const shellClass =
    inRevertImpactZone && !isReopening
      ? 'border border-red-500/55 bg-red-950/45 shadow-[0_0_32px_rgba(239,68,68,0.22)]'
      : isSuccess
        ? 'bg-emerald-950/35 shadow-[0_0_28px_rgba(16,185,129,0.18)]'
        : isRunning
          ? 'bg-blue-950/35 shadow-[0_0_28px_rgba(59,130,246,0.2)]'
          : showWaitingVisual
            ? 'bg-cyan-950/30 shadow-[0_0_26px_rgba(34,211,238,0.14)]'
            : isError
              ? 'bg-rose-950/32 shadow-[0_0_24px_rgba(244,63,94,0.14)]'
              : 'bg-zinc-900/50 hover:bg-zinc-900/65';

  const statusIcon = showWaitingVisual ? (
    <Clock className="h-[18px] w-[18px] text-cyan-400" strokeWidth={2} aria-hidden />
  ) : isPendingOnly ? (
    <CircleDashed className="h-[18px] w-[18px] text-zinc-400" strokeWidth={2} aria-hidden />
  ) : isRunning ? (
  ...
  return (
    <div
      ...
      className={`absolute w-60 max-w-[92vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-transparent backdrop-blur-xl transition-all duration-300 group ${shellClass} ${
    ...
    >
      ...
      <div
        className={`pointer-events-none absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-xl ${
          inRevertImpactZone && !isReopening ? 'sf-workflow-revert-corner bg-red-950/50' : 'sf-workflow-node-dim'
        }`}
        aria-hidden
      >
        {inRevertImpactZone && !isReopening ? (
          <AlertTriangle className="h-[18px] w-[18px] text-red-400" strokeWidth={2} aria-hidden />
        ) : (
          statusIcon
        )}
      </div>
      ...
      <div className="flex items-start gap-3 pl-5 pr-14 pb-2 pt-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[15px] font-bold leading-snug tracking-wide text-zinc-50 sf-text-primary">{step.label}</div>
          <p className="mt-2 text-[11px] leading-relaxed text-zinc-500 sf-text-secondary">{description}</p>
        </div>
      </div>
          ...
          {isReopening ? (
            ...
            <div className="flex flex-wrap items-stretch gap-2">
              <button
                ...
                className="min-h-[34px] flex-1 rounded-lg bg-zinc-800/95 px-3 text-[11px] font-medium text-zinc-200 transition-colors hover:bg-zinc-700"
              >
                取消
              </button>
              <button
            ...
            <button
              ...
              className="w-full rounded-lg border border-zinc-600/80 bg-zinc-800/60 px-3 py-2 text-[11px] font-medium text-zinc-200 transition-colors hover:border-zinc-500 hover:bg-zinc-800"
            >
              回退
            </button>
          )}
  ...
  const content = (
    ...
    >
      <div
        ...
        className="flex max-h-[min(92vh,900px)] w-[min(1024px,calc(100vw-2rem))] flex-col overflow-hidden rounded-[24px] border border-zinc-800/90 sf-border-base bg-zinc-950/98 sf-bg-elevated shadow-[0_24px_90px_rgba(0,0,0,0.58)] ring-1 ring-zinc-500/30"
      >
        <div className="sf-workflow-dialog-header border-b border-zinc-800/90 sf-border-base bg-gradient-to-br from-zinc-900 via-zinc-950 to-zinc-950 px-5 py-4 sm:px-6 sm:py-5">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-zinc-100">
                <GitBranch className="h-4.5 w-4.5 text-violet-300" />
                <h3 id="workflow-panel-title" className="text-base font-semibold tracking-wide">
                  工作流面板
                </h3>
              </div>
              <p className="mt-1.5 text-sm leading-relaxed text-zinc-400">{recommendation}</p>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              ...
              <button
                ...
                aria-label="关闭工作流面板"
                className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-700/80 bg-zinc-900/75 text-zinc-400 transition-colors hover:border-zinc-600 hover:bg-zinc-800 hover:text-zinc-100"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
        ...
        <div className="sf-scrollbar-visible min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
            <div className="sf-workflow-canvas relative aspect-[21/9] w-full overflow-hidden rounded-3xl bg-zinc-950 shadow-2xl">
            <div
                  ...
                  <g key={`${from}-${to}`}>
                    ...
                    {edgeInRevertImpact ? (
                      <path
                        ...
                        strokeLinecap="round"
                        opacity={0.95}
                        className="drop-shadow-[0_0_4px_rgba(248,113,113,0.55)]"
                      />
                    ) : isActive ? (
                      <path
                        d={pathD}
                        fill="none"
                        stroke={isFlowing ? '#3B82F6' : '#10B981'}
                        strokeWidth="0.6"
        ...
        <div className="border-t border-zinc-800/90 bg-zinc-950/90 px-5 py-3 sm:px-6">
