import React, { Fragment, useEffect, useRef, useState, type ReactNode, type RefObject } from 'react';
import {
  Play,
  Pause,
  SkipBack,
...
export type TimelineWorkflowPanelConfig = {
  steps: WorkflowStep[];
...
interface TimelineProps {
  ...
  /** 为 true 时仅隐藏轨道区；顶栏播放/字幕/侧栏/工作流仍可用 */
  ...
  /** 底部时间轴工具栏：工作流面板（与字幕/侧栏按钮同一行） */
  workflowPanel?: TimelineWorkflowPanelConfig | null;
  /** 递增时关闭已打开的工作流面板（手动确认弹窗已把执行交给后台时） */
  workflowPanelDismissSignal?: number;
}
...
export function Timeline({
  ...
  workflowPanel = null,
  workflowPanelDismissSignal = 0,
}: TimelineProps) {
  const timelineRef = useRef<HTMLDivElement>(null);
  const [workflowOpen, setWorkflowOpen] = useState(false);
  const workflowRef = useRef<HTMLDivElement>(null);
  const lastWorkflowDismissSignal = useRef(0);
  const showWorkflowToggle =
    Boolean(workflowPanel?.steps?.length);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!t) return;
      // 分支按钮区域
      if (workflowRef.current?.contains(t)) return;
      // 面板通过 portal 挂在 body，不在 workflowRef 内；须按标记识别
      if (t.closest('[data-workflow-panel-root]')) return;
  ...
  useEffect(() => {
    if (workflowPanelDismissSignal <= 0) return;
    if (workflowPanelDismissSignal === lastWorkflowDismissSignal.current) return;
    lastWorkflowDismissSignal.current = workflowPanelDismissSignal;
    setWorkflowOpen(false);
  }, [workflowPanelDismissSignal]);
  ...
  return (
    <div
      style={{ height }}
      className="sf-editor-timeline flex w-full min-h-0 min-w-0 shrink-0 flex-col border-t border-zinc-800 bg-zinc-900/80 backdrop-blur-md z-30"
    >
      <div className="flex h-12 min-h-12 min-w-0 shrink-0 items-center justify-between gap-2 border-b border-zinc-800 bg-zinc-950/50 px-2 sm:px-4">
        <div className="flex min-w-0 shrink-0 items-center gap-0.5 sm:gap-1">
          ...
          <button
            ...
            className="shrink-0 rounded-md p-2 text-zinc-100 sf-text-primary transition-colors hover:bg-zinc-800 hover:text-white"
          >
          ...
          <div className="sf-chip-neutral ml-2 shrink-0 whitespace-nowrap rounded border px-2 py-1 font-mono text-[10px] shadow-inner sm:ml-4 sm:px-3 sm:text-xs">
        ...
        <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
          ...
          <button
            ...
            className={[
              'shrink-0 rounded-md p-2 transition-colors',
              !leftDetailCollapsed
                ? 'border border-zinc-600/50 bg-zinc-800/80 text-zinc-200 hover:bg-zinc-800'
                : 'border border-transparent text-zinc-400 hover:bg-zinc-800 hover:text-white',
            ].join(' ')}
          >
            <LayoutPanelLeft className="h-4 w-4" />
          </button>
          {showWorkflowToggle && workflowPanel ? (
            <div className="relative shrink-0" ref={workflowRef}>
              <button
                type="button"
                onClick={() => setWorkflowOpen((cur) => !cur)}
                title="打开工作流面板"
                aria-label="打开工作流面板"
                aria-expanded={workflowOpen}
                className={[
                  'inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40',
                  workflowOpen
                    ? 'border-violet-500/40 bg-violet-500/12 text-violet-100'
                    : 'border-zinc-700/80 bg-zinc-900/75 text-zinc-400 hover:border-zinc-600 hover:bg-zinc-800/90 hover:text-zinc-100',
                ].join(' ')}
              >
              ...
              {workflowOpen ? (
                <WorkflowPanel
                  ...
                  pipelineAutoAdvance={workflowPanel.pipelineAutoAdvance}
                  manualOutlineConfirmed={workflowPanel.manualOutlineConfirmed}
                  deckMasterSourceProjectId={workflowPanel.deckMasterSourceProjectId}
                  videoReady={workflowPanel.videoReady}
                  onRetryStep={workflowPanel.onRetryStep}
                  retryingStepId={workflowPanel.retryingStepId}
                  onCancelRunningStep={workflowPanel.onCancelRunningStep}
                  cancellingStepId={workflowPanel.cancellingStepId}
                  onCommitReopenSuccessStep={workflowPanel.onCommitReopenSuccessStep}
                  reopeningStepId={workflowPanel.reopeningStepId}
      ...
      {/* 轨道区：生成中/无成片时可隐藏，顶栏菜单始终保留 */}
      {isGenerating ? (
        <div className="flex min-h-0 min-w-0 flex-1 flex-col items-center justify-center overflow-hidden border-t border-zinc-800/80 bg-zinc-950/40 px-4 text-center text-sm text-zinc-500">
          时间轴将在音频与场景就绪后显示
        </div>
      ) : (
