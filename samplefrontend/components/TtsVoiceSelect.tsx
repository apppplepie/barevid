    ...
    open && !emptyOrLoading && menuRect
      ? createPortal(
          <div
            ...
            role="listbox"
            className="sf-scrollbar-visible fixed z-[200] overflow-y-auto overflow-x-hidden overscroll-contain rounded-xl border border-zinc-700/90 bg-zinc-950/98 py-1 shadow-2xl ring-1 ring-purple-500/10 backdrop-blur-md"
                    ...
                    <button
                      ...
                      className={`flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors ${
                        selected
                          ...
                          : 'text-zinc-200 hover:bg-zinc-800/95 hover:text-zinc-50'
                      }`}
                      onClick={() => {
                        onChange(opt.value);
                        setOpen(false);
  ...
  return (
    <div ref={rootRef} className="relative w-full max-w-md">
      <button
        ...
        className={`flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left text-sm transition-[border-color,box-shadow,background-color] ${
          open
            ? 'border-purple-500/50 bg-zinc-900/90 sf-bg-card text-zinc-100 sf-text-primary shadow-[0_0_0_1px_rgba(168,85,247,0.2)] ring-2 ring-purple-500/25'
            : 'border-zinc-800/90 bg-zinc-950/80 text-zinc-100 shadow-inner hover:border-zinc-700/90 hover:bg-zinc-900/60'
      ...
      >
        ...
        <ChevronDown
          className={`h-4 w-4 shrink-0 text-zinc-500 transition-transform duration-200 ${
            open ? 'rotate-180 text-purple-400/90' : ''
          }`}
          aria-hidden
        />
      </button>
