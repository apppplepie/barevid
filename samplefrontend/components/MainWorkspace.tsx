...
import { PageRenderer, computeDeckPageIdleOverlay } from './PageRenderer';
import {
...
export function MainWorkspace({
  ...
  const activeGateStep =
    ...
    gateSteps.find((s) => s.state === 'waiting') ||
  ...
  const deckPageIdleOverlay = useMemo(
    () => computeDeckPageIdleOverlay(steps),
    [steps],
  );
  ...
  if (showInitialPipelineGate) {
    ...
    const isWaitingGate = activeGateStep.state === 'waiting';
    const isRunningGate = activeGateStep.state === 'running';
    return (
      <div className="sf-main-workspace sf-main-gate-screen flex min-h-0 min-w-0 flex-1 flex-col items-center justify-center bg-zinc-950 sf-bg-base relative">
        <div className="flex flex-col items-center gap-4">
          {isError ? (
            ...
            <div className="sf-main-gate-waiting flex h-14 w-14 items-center justify-center rounded-full border border-cyan-500/35 bg-cyan-950/40 text-cyan-200">
          ...
          <div className="text-center">
            <h3 className="sf-main-gate-title text-lg font-medium text-zinc-200 sf-text-primary">
              {isError
                ...
                : isWaitingGate
                  ...
                  : isRunningGate
                    ...
                    : `准备中：${activeGateStep.label}`}
            </h3>
            <p className="sf-main-gate-desc text-sm text-zinc-500 sf-text-secondary mt-1">
              {isError
                ? '可在顶部进度条该步骤上点击重试。'
                : isWaitingGate
                  ? '可在顶部进度条点击该步骤上的闹钟图标，按引导开始。'
                  : '可能需要一点时间。'}
            </p>
          </div>
        </div>
    ...
    return (
      <div className="sf-main-workspace sf-main-gate-screen flex min-h-0 min-w-0 flex-1 flex-col items-center justify-center bg-zinc-950 sf-bg-base relative">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            <h3 className="sf-main-gate-title text-lg font-medium text-zinc-200 sf-text-primary">加载时间轴...</h3>
            <p className="sf-main-gate-desc text-sm text-zinc-500 sf-text-secondary mt-1">从服务器同步音频片段...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`sf-main-workspace flex min-h-0 min-w-0 flex-1 flex-col bg-zinc-950 relative ${previewNoScrollbar}`}
    >
      {/* Preview Area */}
      <div
      ...
      >
        <div
          ref={previewShellRef}
          className={`sf-main-preview-shell relative group flex min-h-0 min-w-0 flex-1 items-center justify-center overflow-hidden border border-zinc-800 sf-bg-elevated shadow-2xl ${previewNoScrollbar} ${
            isPreviewFullscreen ? 'rounded-none' : 'rounded-xl'
          }`}
        >
          {/* Web Page Preview Surface */}
          <div className="sf-main-preview-overlay absolute inset-0 bg-gradient-to-br from-zinc-900 to-zinc-950 opacity-70" />
          ...
          >
              ...
              {activePage ? (
                <div className="h-full w-full min-h-0 min-w-0 max-h-full max-w-full overflow-hidden [&_.sf-neoncast-deck-iframe]:pointer-events-none">
                  <PageRenderer
                    ...
                    deckPageIdleOverlay={deckPageIdleOverlay}
                  />
                </div>
              ) : (
                <div className="sf-main-empty-state flex flex-col items-center gap-3 text-zinc-600">
                  <MonitorPlay className="sf-main-empty-icon w-16 h-16 text-zinc-700" />
            ...
            {activeAudioClip && (
              <div className="sf-main-audio-chip sf-chip-ready pointer-events-none absolute top-3 left-3 z-10 max-w-[calc(100%-1.5rem)] truncate rounded-full border px-2.5 py-1 text-[11px] font-mono backdrop-blur sm:top-4 sm:left-4 sm:px-3 sm:text-xs">
            ...
            {showSubtitles && subtitleLine ? (
              ...
              >
                <p className="sf-main-subtitle-chip rounded-lg px-3 py-2 text-center text-xs leading-relaxed text-zinc-100 shadow-lg backdrop-blur-md sm:px-4 sm:text-base">
                  {subtitleLine}
                </p>
              </div>
            ) : null}
          ...
          <div className="absolute bottom-3 left-3 right-3 z-20 flex min-w-0 items-center justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100 sm:bottom-4 sm:left-4 sm:right-4">
            ...
            */}
            <button
              ...
              className="sf-main-preview-control shrink-0 rounded-lg border border-zinc-700/50 bg-zinc-900/80 p-1.5 text-zinc-300 backdrop-blur-md transition-colors hover:bg-zinc-700 hover:text-white"
            >
              {isPreviewFullscreen ? (
          ...
          {isPreviewFullscreen && playbackReady ? (
            <div className="sf-main-fullscreen-bar pointer-events-auto absolute inset-x-0 bottom-0 z-40 flex min-w-0 items-center gap-3 border-t border-zinc-800/80 bg-zinc-950/90 px-4 py-3 backdrop-blur-md sm:gap-4 sm:px-6 sm:py-4">
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                ...
                className="sf-main-progress-track relative h-2 w-full cursor-pointer rounded-full bg-zinc-800"
                >
                  <div
                    className="sf-main-progress-fill absolute left-0 top-0 h-full rounded-full bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]"
              ...
              <button
                ...
                title="退出全屏"
                className="sf-main-preview-control shrink-0 rounded-lg border border-zinc-700/60 bg-zinc-900/90 p-2 text-zinc-300 transition-colors hover:bg-zinc-800 hover:text-white"
              >
                <Minimize2 className="h-4 w-4" />
              </button>
            </div>
