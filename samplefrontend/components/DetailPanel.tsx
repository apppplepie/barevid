...
export function DetailPanel({
  ...
  onNotify,
  playback,
  /** 口播助理中的未应用草稿（与已保存台词不同时左侧展示提示与草稿正文） */
  audioNarrationDraftText,
}: {
  width?: number;
  clip: ClipData;
  ...
  audioNarrationDraftText?: string | null;
}) {
  if (isGenerating) {
    return (
      <div
        ...
        {surface === 'audio' ? (
          <Fragment key={`a-${clip.id}`}>
            <AudioDetails
              ...
              onNotify={onNotify}
              playback={playback}
              narrationDraftText={audioNarrationDraftText ?? null}
            />
          </Fragment>
        ) : (
...
function AudioDetails({
  ...
  narrationDraftText?: string | null;
}) {
  ...
  const serverNarrationLine = (step?.narration_text ?? clip.content ?? '').trim();
  const draftLine = (narrationDraftText ?? '').trim();
  const draftDiffersFromSaved =
    narrationDraftText != null && draftLine !== serverNarrationLine;
  ...
  return (
    ...
    >
      <div className="flex shrink-0 min-w-0 items-center gap-2">
        ...
        <button
          type="button"
          disabled={resynthBusy || draftDiffersFromSaved}
          title={
            draftDiffersFromSaved
              ? '存在未应用口播草稿，请在右侧口播助理中重新生成或先丢弃草稿'
              : undefined
          }
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3">
        ...
        {draftDiffersFromSaved ? (
          <div className="sf-detail-amber-hint shrink-0 rounded border border-amber-500/35 bg-amber-950/30 px-2.5 py-2 text-[11px] leading-snug text-amber-100/95">
            口播助理中有未应用草稿。下方纯文本区展示草稿全文；若有句级时间轴，其仍对应当前已合成的音频时间轴。
          </div>
        ) : null}
              ...
              <div className="flex flex-col gap-2">
                {sentenceCues.map((cue, idx) => (
                  <div
                    ...
                    className={
                      idx === activeCueIndex
                        ? 'scroll-mt-0 rounded border border-purple-500/30 bg-purple-500/20 px-1.5 py-0.5 text-zinc-100 sf-text-primary'
...
function PageDetails({
  ...
  }, [projectId, clip.type, clip.pageId, currentPageNodeId, onDeckPageRegenSubmitted, onNotify]);
          ...
          {clip.nodes?.map((node) => (
            ...
            >
              ...
              <div className="relative z-10 flex h-2.5 w-2.5 shrink-0 items-center justify-center">
                <div className="sf-chip-neutral h-2 w-2 rounded-full border transition-all duration-150 group-hover:border-blue-400 group-hover:bg-blue-500/20" />
              </div>
