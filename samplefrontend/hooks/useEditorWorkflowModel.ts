...
type UseEditorWorkflowModelArgs = {
  ...
  /** POST /workflow/text/run 已返回但后台尚未把 DB 写成 running 的窗口，避免仍显示旧的 success */
...
export function useEditorWorkflowModel({
  ...
  /** 手动流水线进入编辑器时按顺序弹窗：须以 workflowSteps 为准，不能只看 pipeline.outline（否则 text 仍为 pending 时会误弹口播确认）。 */
  useEffect(() => {
  ...
  const displaySteps = useMemo(() => {
    // 单页重新生成时不要把整个「场景页面」标成 running，否则顶栏/流程图里母版→场景的连线会播放流动动画，像母版在动；进度以侧栏该页状态为准。
  ...
  const videoReady = Boolean(currentProject?.pipeline?.video) &&
    !headerAudioRegenPending &&
    !headerDeckRegenPending &&
    !headerExportStaleAfterRegen;
