...
async def clone_project_deep(
    ...
    *,
    ...
    new_proj.export_file_url = remap_project_media_refs(src.export_file_url, old_pid, new_pid)
    new_proj.audio_result_url = remap_project_media_refs(src.audio_result_url, old_pid, new_pid)
    new_proj.text_result_url = remap_project_media_refs(src.text_result_url, old_pid, new_pid)
    new_proj.demo_result_url = remap_project_media_refs(src.demo_result_url, old_pid, new_pid)
    new_proj.deck_json = remap_project_media_refs(src.deck_json, old_pid, new_pid)
    session.add(new_proj)
    ...
    try:
        st_res = await session.exec(
            select(ProjectStyle).where(ProjectStyle.project_id == source_project_id)
        )
        st = st_res.first()
        if st is not None:
            session.add(
                ProjectStyle(
                    project_id=new_pid,
                    style_preset=(st.style_preset or "none").strip() or "none",
                    user_style_hint=st.user_style_hint,
                    style_prompt_text=st.style_prompt_text or "",
                    style_data_json=remap_project_media_refs(st.style_data_json, old_pid, new_pid),
                    style_base_json=(
                        remap_project_media_refs(st.style_base_json, old_pid, new_pid) or ""
                    ),
                    version=int(st.version or 1),
                    created_at=now,
                    updated_at=now,
                )
            )
